...
export interface IHeaderState {
  shouldDisplayUserPanel: boolean,
  menus: Array<IMenu>,
  activeMenu: string,
}
...
const UserPanelMobile =  (props: any) => {
  if (props.shouldDisplay) {
    return (     
...
const Menus = (props: { menus: Array<IMenu>, activeMenu: string, setMenu: Function }) => {

  const Links = props.menus.map(menu => {
    if (menu.name === props.activeMenu) {
      return (
        <a href={menu.path} onClick={ () => { props.setMenu(menu.name) }} key={menu.name} className="bg-gray-900 text-white px-3 py-2 rounded-md text-sm font-medium">{ menu.name }</a>
      )
    }

    return (
      <a href={menu.path} onClick={ () => { props.setMenu(menu.name) }} key={menu.name} className="text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">{ menu.name }</a>
    )
  })
...
const App = () => {
  ...
  const [shouldDisplayUserPanel, setShouldDisplayUserPanel ] = useState(false)
  const [activeMenu, setActiveMenu ] = useState('Dashboard')
  const handleClickAvatar = () => {
    setShouldDisplayUserPanel(!shouldDisplayUserPanel)
  }
  ...
  return (
    <nav className="bg-gray-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            ...
            <Menus menus={ menus } activeMenu={ activeMenu } setMenu={handleSetMenu}/>
          </div>
          <div className="hidden md:block">
            <div className="ml-4 flex items-center md:ml-6">
              ...
              <div className="ml-3 relative">
                ...
                <UserPanel shouldDisplay={shouldDisplayUserPanel} />     
              </div>
            </div>
          </div>
      ...
      <div className="pt-4 pb-3 border-t border-gray-700">
        <div  onClick={handleClickAvatar}  className="flex items-center px-5">
          <div className="flex-shrink-0">
        ...
        <UserPanelMobile shouldDisplay={shouldDisplayUserPanel} />
      </div>
    </div>
  </nav>
  );
